...
class ExpertCache:
    ...
    def get_or_load(
        ...
        layer: int,
        expert_id: int,
        loader: Callable[[], Mapping[str, bytes]],
    ) -> CachedExpert:
        cached = self.lookup(layer, expert_id)
        if cached is not None:
            return cached

        payloads = dict(loader())
        if not payloads:
            raise ValueError(f"loader returned no payloads for layer={layer}, expert={expert_id}")
        entry = CachedExpert(layer=layer, expert_id=expert_id, parts=payloads)
        self._insert(entry)
    ...
    def _insert(self, entry: CachedExpert) -> None:
        ...
        while len(layer_cache) >= self.capacity_per_layer:
