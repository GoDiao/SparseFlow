...
### Phase 7.5.2 status: INT8 expert container complete

`canonical-int8-v1` 已完整转换 Qwen3.6 的 40 层/10,240 experts：60.000 GiB
BF16 routed weights 变为 30.078 GiB physical（50.13%），转换峰值 RSS
...
### Phase 7.5.4 status: native INT8 kernel complete

已实现 canonical-row-major AVX-512 VNNI W8A8 expert kernel，不在 streaming
miss 上做权重 prepack。Resident/streaming 32-token logits、routes、IDs、text
exact；三次 paired 运行中 native resident/streaming 相对 W8A16 reference
分别达到 `1.270x` / `1.241x` decode speedup。W8A8 相对 W8A16 的独立质量
边界为 32/32 argmax equal、无 greedy 分叉。结果见
`docs/results/qwen36_stage7_5_4_native_int8_20260716.md`。[Main Dev]

下一步 Phase 7.5.5 在 INT8 expert 大小和 native kernel 下重新 sweep
